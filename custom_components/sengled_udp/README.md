...
5. Choose:
   - **Discover**: Select bulbs from the list using checkboxes, or
   - **Manual**: Enter one or more bulb IPs (any of these formats work):
     - one per line
     - comma-separated
6. Set **Name prefix** (defaults to `Sengled`) and finish. If you add multiple bulbs at once, names will look like `Sengled (RGB) (192.168.x.x)`.
